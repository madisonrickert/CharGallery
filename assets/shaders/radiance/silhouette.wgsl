...
//                slot channels grayscale), w the aspect-fit x scale
//                (mirrors fit_params.x).
...
//   @binding(6): fit_params — xy the aspect-fit UV remap scales
//                (window/mask_rect per axis, from
//                systems::sim_params::mask_fit_uv_scale), zw reserved.
...
@group(2) @binding(6) var<uniform> fit_params: vec4<f32>;
...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // so fill, rim, and particle spawns agree. fit_params.xy is the aspect-fit
    // remap (window/mask_rect per axis): scaling BOTH axes about the centre
    // places the camera frame at its true proportions on any display aspect —
    // a scale of 1 spans that axis, > 1 boxes it (margins), < 1 crops it.
    // Both axes matter: with Fit on a portrait screen the box is vertical, so
    // remapping u alone would leave the letterbox bands unmasked.
    var uv = in.uv;
    if (fill_params.w > 0.5) {
        uv.x = 1.0 - uv.x;
    }
    uv = vec2<f32>(0.5) + (uv - vec2<f32>(0.5)) * fit_params.xy;
    // Outside the frame reads as background — the letterbox/pillarbox margin.
    // A cropped axis never lands here (its samples stay inside [0,1]).
