...
- GPU resources are released by three distinct mechanisms, and it matters which one applies:
  ...
  2. **Render-world `Resource`s** are *not* touched by an entity despawn. `ExtractResourcePlugin` does not propagate removals, so each one needs an explicit removal system (see `remove_particle_sim_params_if_absent` in `wc-sketches/src/particles/compute.rs` — a module shared by Line and Dots — and its siblings `remove_line_post_params_if_absent`, `remove_dots_post_params_if_absent`, `remove_cymatics_sim_params_if_absent`, `remove_flame_sim_params_if_absent`, `remove_hand_mesh_target_if_absent`).
...
- An 8-hour soak test is required before any release tag, on the target deployment hardware. It is automated: `cargo xtask soak-test` (see **Soak testing** below).

## Soak testing

- `cargo xtask soak-test` runs the app under representative load (hand tracking + audio active, sketch cycling), samples RSS / FPS / thermal tier on a fixed interval, fits leak and decay trends, and writes a self-describing artifact to `target/soak/<label>/` (`run.json` + `samples.ndjson` + `app.log` + the app's last `health.json`). Build the app first (`cargo build -p waveconductor`); soak-test launches the pre-built debug binary and fails fast with that directive if it is missing, exactly as `capture` does.
  - Pre-tag gate: `cargo xtask soak-test` (defaults to 8 h, sampling every 30 s, cycling sketches every 5 m).
  - `--silent` hard-mutes the app's master audio for the whole run (the mute rides `WC_SOAK` into the app; a soak in a shared living space must not play synths all night). The DSP graph still ticks, so the load stays representative.
  - Smoke the harness first: `cargo xtask soak-test --duration 2m --sample 5s --cycle 20s --warmup 10s`. A run this short **cannot** produce a PASS: it is far too short for the fit to resolve a leak, and it says so. It validates the *harness*, not the *build*.
  - Review a finished run without relaunching: `cargo xtask soak-test --report target/soak/<label>`. Add `--json` to any invocation for a machine-readable report. `cargo xtask soak-test --help` documents the rest.
- Three verdicts, and the distinction is deliberate. **FAIL** = a mechanically-certain failure (the app died, froze, panicked on any thread, held a single frame past `hitch_fail_ms`, climbed in RSS on a steep well-fit slope, or decayed in FPS past the threshold); exits nonzero. **REVIEW REQUIRED** = the data is real but the judgment is not mechanical (a mild upward drift that could be a bounded cache settling, an `ERROR` line in the log, a sub-second hitch, too few samples to fit, or a run too short to resolve a leak at all); the tool prints the numbers and a directive naming the artifact, and does **not** fabricate a verdict. When you see it, read `samples.ndjson` and judge — that is the intended agent-in-the-loop step, and there is no LLM API spend anywhere in the harness.
- **What PASS actually means.** *This build, on this hardware, in a debug profile, ran the configured duration to a clean exit; no thread panicked and no `ERROR` line reached the log; the app's clock never stalled and no single frame exceeded 2 s; RSS showed no well-fit climb the run was long enough to resolve; and active-sketch FPS did not decay.* It does **not** mean "no leak". The tool is blind to all of the following, and a release decision has to account for them by other means:
  - **A leak that begins late in the run.** The fit is over the whole post-warmup span, so a 40 MiB/h leak starting at hour 7 of an 8 h run fits to ~1.7 MiB/h with a delta under the noise floor, and passes. (Not mitigated — the "also fit the last half" refinement was considered and deliberately not implemented. If a build is suspected of a late leak, plot `samples.ndjson` by eye.)
  - **Any sustained leak below ~5 MiB/h** (the review slope) — about 40 MiB over 8 h. Under the noise floor, under every branch.
  - **GPU/VRAM memory, which is not sampled at all.** The most likely place for this project's leaks to live (unreleased render-world resources across sketch transitions) is the one place the harness does not look. Watch it by hand in Activity Monitor / `nvtop` during a gate run.
  - **Absolute numbers.** The soak measures a **debug** binary, so its RSS and FPS are not release figures. It is looking for *drift*, not for levels.
  - **A backgrounded window.** The run needs the app window foregrounded and display/system sleep disabled. An occluded or asleep window throttles its present rate and reads as FPS decay — a false FAIL. (Same environment constraint as `cargo xtask capture`.)
- The app-side half is `WC_SOAK` (`wc-core`'s `soak` module): it republishes a `health.json` snapshot (including a **per-interval frame-time high-water mark** — the only lane that can see a wedge shorter than one launcher sample; `app_uptime_secs` advances right through a 25 s hitch in a 30 s window), advances the sketch on the cycle interval, and self-exits at the duration. Like `capture`, it is `#[cfg(debug_assertions)]`-gated. The launcher additionally scans the teed `app.log`: a debug build **unwinds** on panic, so a panic on a worker thread (inference, audio watcher) kills that thread and leaves the process rendering happily — every metric lane reports perfect health, and only the log knows. RSS is measured from outside the process (`ps`), so no dependency was added.
